...
class V2AviTrafficRollback(AviConverter):
    ...
    def initiate_rollback_vs(self):
        ...
        try:
            ...
            nsxv_util = NsxvUtil(self.nsxv_ip, self.nsxv_user, self.nsxv_password, self.input_path, None, None, None, None,
                                 c_ip=self.alb_controller_ip, c_user=self.alb_controller_user,
                                 c_pw=self.alb_controller_password, c_vr=self.alb_controller_version)
            vedge_lb_config = nsxv_util.get_nsxv_Edges()

            nsxv_vs_not_found, alb_vs_not_found = nsxv_util.rollback_vs(vedge_lb_config, self.edge, self.vs_filter,
            ...
            if alb_vs_not_found:
                print_msg = "\033[93m" + "\nWarning: Cannot perform rollback for following VS as it is not found " \
                                         "on ALB" + "\033[0m"
                print(print_msg)
            ...
            msg = f"Error while performing traffic rollback. Message: {e}"
            print(msg)
            LOG.error(msg)
            sys.exit(1)
...
if __name__ == "__main__":
    HELP_STR = """
    Usage:
    python v2avi_traffic_rollback.py -n 192.168.100.101 -u admin -p password --alb_controller_ip 10.20.30.40 
    ...
    parser.add_argument('--edge',
                        help='NSX-V edge for which traffic rollback is to be executed.\n',
                        required=True)
    parser.add_argument('--prefix', help='Prefix used while migrating the NSX-V load balancer config')
    parser.add_argument('--vs_filter', help='comma separated names of virtual services within the specified edge '
                                            'for performing rollback.\n', required=True)
    parser.add_argument('-o', '--output_file_path', help='Folder path for output files to be created in')

    start = datetime.now()
    args = parser.parse_args()

    traffic_rollback = V2AviTrafficRollback(args)
    traffic_rollback.initiate_rollback_vs()
