...
class F5Converter(AviConverter):
    ...
    def convert(self):
        ...
        for command in merged_not_supported_list:
            ...
            object_name = f"{ object_type[-1]}/{d[-1]}"
    ...
    def get_default_config(self, is_download, path):
        ...
        if is_download:
            ...
            with open(dir_path + os.path.sep + f"f5_v{self.f5_config_version}_defaults.conf", "r") \
                    as defaults_file:
                if self.skip_default_file:
                    LOG.warning(
    ...
    if terminal_args.args_config_file:
        with open(terminal_args.args_config_file) as file:
            ...
            if config_file:
                ...
                for argument, argument_list in ARG_CHOICES.items():
                    if terminal_args.__dict__[argument] not in argument_list:
                        msg = (
                            f"{parser.prog}: error:\
                                argument --{argument}: invalid choice: "
                            "'{terminal_args.__dict__[argument]}'\
                                (choose from {argument_list})"
                        )
                        LOG.debug(msg)
                        print(msg)
                        exit(1)
...
if __name__ == "__main__":
    ...
    if args.version:
        print(
            f"SDK Version: {sdk_version}\nController Version: {args.controller_version}"
        )
        exit(0)
