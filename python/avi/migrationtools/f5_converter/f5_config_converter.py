    ...
    for key in avi_config_dict.keys():
        if key != "META":
            if key == "VirtualService":
                if vs_level_status:
                    LOG.info(
                        "Total Objects of %s : %s (%s full conversions)", key,
                        len(avi_config_dict[key]), conversion_util.fully_migrated
                    )
                    print(
                        f"Total Objects of {key}: {len(avi_config_dict[key])}({conversion_util.fully_migrated} full conversions)")
                else:
                    LOG.info(
                        "Total Objects of %s : %s", key, len(
                            avi_config_dict[key]))
                    print(
                        f"Total Objects of {key} : {len(avi_config_dict[key])}")
            ...
            if object_merge_check and key == "SSLProfile":
                ...
                profile_merged_message = f"Total Objects of {key}: {len(avi_config_dict[key])}\
                        ({abs(mergedfile)}/{ssl_count['count']} profile merged)"
            ...
            if object_merge_check and key == "ApplicationProfile":
                ...
                profile_merged_message = f"Total Objects of {key} : {len(avi_config_dict[key])}\
                        ({abs(mergedfile)}/{ profile_conv.app_count}\
                            profile merged)"
                LOG.info(profile_merged_message)
                print(profile_merged_message)
                continue
            if object_merge_check and key == "NetworkProfile":
                ...
                profile_merged_message = f"Total Objects of {key} : {len(avi_config_dict[key])}\
                        ({abs(mergedfile)}/{profile_conv.net_count}\
                            profile merged)"
                LOG.info(profile_merged_message)
                print(profile_merged_message)
                continue
            if object_merge_check and key == "HealthMonitor":
                ...
                monitor_merged_message = f"Total Objects of {key} : {len(avi_config_dict[key])}\
                        ({ abs(mergedmon)}/{mon_conv.mon_count} monitor merged)"
            ...
            if object_merge_check and key == "PKIProfile":
                ...
                profile_merged_message = f"Total Objects of {key} : {len(avi_config_dict[key])}\
                        ({abs(mergedfile)}/{profile_conv.pki_count}\
                            profile merged)"
                LOG.info(profile_merged_message)
                print(profile_merged_message)
                continue
            if object_merge_check and key == "ApplicationPersistenceProfile":
                ...
                profile_merged_message = f"Total Objects of {key} : {len(avi_config_dict[key])}\
                        ({abs(mergedfile)}/{ persist_conv.app_per_count}\
                            profile merged)"
                LOG.info(profile_merged_message)
                print(profile_merged_message)
                continue
            if object_merge_check and key == "SSLKeyAndCertificate":
                ...
                certkey_merged_message = f"Total Objects of {key} : {len(avi_config_dict[key])} \
                        ({abs(mergedfile)}/{profile_conv.certkey_count}\
                            cert key merged)"
