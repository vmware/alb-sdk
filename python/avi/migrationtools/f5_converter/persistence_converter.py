...
class PersistenceConfigConv(object):
    @classmethod
    def get_instance(
        cls, version, f5_persistence_attributes, prefix, object_merge_check
    ):
        """
    ...
    def update_conversion_status(
        self, conv_status, persist_mode, name, persist_profile, f5_profile
    ):
        """
    ...
    def convert(
        ...
        for key in f5_persistence_dict.keys():
            ...
            if key == "global-settings":
                msg = "persist mode not supported skipping conversion: %s" % name
                LOG.warning(msg)
                strMsg = dict()
                strMsg["warning"] = msg
                self.update_conv_status_for_skip(None, name, strMsg)
                continue
            try:
                ...
                if persist_mode == "cookie":
                    ...
                    LOG.warn(
                        "hash-persistence profile %s will be mapped "
                        "indirectly to Pool -> Load Balance  Algorithm" % name
                    )
                    conv_status = {
                    ...
                    conv_utils.add_conv_status(
                        "profile", "hash-persistence", name, conv_status, strMsg
                    )
                    continue
                else:
                    msg = "persist mode not supported skipping conversion: %s" % name
                    ...
                    strMsg = dict()
                    strMsg["warning"] = msg
                    self.update_conv_status_for_skip(persist_mode, name, strMsg)
                    continue
                if not persist_profile:
                    continue
                ...
                f5_of_avi["ApplicationPersistenceProfile"].update(
                    {
                        persist_profile["name"]: {
                            "F5_ID": name,
                            "F5_type": "persistence",
                            "F5_SubType": persist_mode,
                        }
                    }
                )
                ...
                update_count("error")
                LOG.error(
                    "Failed to convert persistance profile : %s" % key, exc_info=True
                )
...
class PersistenceConfigConvV11(PersistenceConfigConv):
    def __init__(self, f5_persistence_attributes, prefix, object_merge_check):
        ...
        self.supported_attr_convert = f5_persistence_attributes[
            "Persistence_" "supported_attr_" "convert_source_addr"
        ]
    ...
    def convert_cookie(self, name, profile, skipped, tenant):
        ...
        skipped += [attr for attr in profile.keys() if attr not in self.supported_attr]
        ...
        persist_profile["tenant_ref"] = conv_utils.get_object_ref(tenant, "tenant")
    ...
    def update_conversion_status(
        ...
        conv_utils.add_conv_status(
            ...
            f5_object=yaml.dump(f5_profile),
        )
    ...
    def update_conv_status_for_error(self, name, persist_mode, key):
        ...
        if name:
            ...
            conv_utils.add_status_row("persistence", key, key, final.STATUS_ERROR)

    def update_conv_status_for_skip(self, persist_mode, name, msg):
        conv_utils.add_status_row(
            "persistence", persist_mode, name, final.STATUS_SKIPPED, msg
        )
...
class PersistenceConfigConvV10(PersistenceConfigConv):
    ...
    def convert_cookie(self, name, profile, skipped, tenant):
        ...
        if not method == "insert":
            LOG.warning("Skipped cookie method not supported for profile '%s' ", name)
    ...
    def update_conv_status_for_error(self, name, persist_mode, key):
        ...
        if name:
            ...
            conv_utils.add_status_row("persistence", "persist", key, final.STATUS_ERROR)
    ...
    def update_conv_status_for_skip(self, persist_mode, name, msg):
        ...
        """
        conv_utils.add_status_row(
            "persistence", "persist", name, final.STATUS_SKIPPED, msg
        )
