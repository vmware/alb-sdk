    ...
    def convert(self, f5_config, avi_config, user_ignore,
        ...
        for key in f5_persistence_dict.keys():
            ...
            if key == "global-settings":
                msg = f"persist mode not supported skipping conversion: {name}"
                LOG.warning(msg)
            ...
            try:
                ...
                update_count("error")
                LOG.error(
                    "Failed to convert persistance profile : %s",
                    key, exc_info=True)
...
class PersistenceConfigConvV11(PersistenceConfigConv):
    ...
    def convert_cookie(self, name, profile, skipped, tenant):
        ...
        if not method == "insert":
            msg = f"Skipped cookie method not supported for profile '{name}' "
            LOG.warning(msg)
