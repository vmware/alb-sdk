        ...
        if "rules" in config and not config["rules"] == "none":
            ...
            for index, each_rule in enumerate(config["rules"]):
                if config["rules"][each_rule].get(
                    ...
                    rule_name = f"{policy_name}-{each_rule}-{str(index + 1)}"
                    rule_dict = {
                        "name": rule_name,
                        "enable": True,
                        "index": index + 1}
                    ...
                    if not match:
                        msg = f"All match conditions not supported for rule {each_rule}\
                    ...
                    if not actions:
                        msg = f"All actions not supported for rule {format} in policy {policy_name}"
                    ...
                    msg = f"Skipping rule:No action found for rule {each_rule}\
                        in policy {policy_name}"

                    LOG.debug(msg)
                    skip_rule[each_rule] = msg
                elif config["rules"][each_rule].get("actions"):
                    msg = f"Skipping rule:No match condition found for rule {each_rule}\
                        in policy {policy_name}"
                    LOG.debug(msg)
                    skip_rule[each_rule] = msg
                else:
                    msg = f"Skipping rule:Match conditions and actions are missing for policy \
                        {policy_name}"
            ...
            msg = f"No rule found for policy {policy_name}"
            LOG.debug(msg)
            return msg
            ...
            if "geoip" in result:
                ...
                if "country-code" in result:
                    if "starts-with" not in result and "contains" not in result \
                        ...
                        ipgrp_name = f"ipaddrgroup{rule_name}-{str(each_index)}"
                        ip_addr_group = {
                ...
                if len(match_criteria) > 1:
                    ...
                    if "not" in match_criteria:
                        cookie["match_criteria"] = f"HDR_{parameters_dict[match_criteria[0]]}EQUAL"
                    else:
                        cookie["match_criteria"] = f"HDR_{parameters_dict[match_criteria[0]]}"
                else:
                ...
                if len(match_criteria) > 1:
                    ...
                    if "not" in match_criteria:
                        header["match_criteria"] = f"HDR_{parameters_dict[match_criteria[0]]}EQUAL"
                    else:
                        header["match_criteria"] = f"HDR_{parameters_dict[match_criteria[0]]}"
                else:
                    ...
                    if len(match_criteria) > 1:
                        ...
                        if "not" in match_criteria:
                            host_header["match_criteria"] = \
                                f"HDR_{parameters_dict[match_criteria[0]]}EQUAL"
                        else:
                            host_header["match_criteria"] = \
                                f"HDR_{parameters_dict[match_criteria[0]]}"
                    else:
                ...
                if "path" in result or "extension" in result:
                    ...
                    if "path" in result:
                        ...
                        if len(match_criteria) > 1:
                            ...
                            if "not" in match_criteria:
                                path_query["match_criteria"] = f"{parameters_dict[match_criteria[0]]}EQUAL"
                            else:
                                path_query["match_criteria"] = f"{parameters_dict[match_criteria[0]]}"
                        else:
                    ...
                    if "contains" in result:
                        strgrp_name = f"stringgroup{rule_name}-{str(each_index)}"
                        string_group = {
                ...
                if "select" in result:
                    if "pool" in result:
                        ...
                        if tenant:
                            p_tenant = tenant
                        if self.prefix:
                            poolname = f"{self.prefix}-{poolname}"
                        poolobj = [
                            obj
                        ...
                        if poolobj:
                            ...
                            pgobj = [
                                ...
                                if poolname == ob["name"] and conv_utils.get_name(obj["tenant_ref"]) == p_tenant
                            ]
                            if pgobj:
