...
class CustomConfig(object):
    ...
    def __init__(self, host, username, password, api_version, tenant):
        self.host = host
    ...
    def ref_clean_up(self, obj):
        ...
        if isinstance(obj, dict):
            return {k: self.ref_clean_up(v) for k, v in obj.items()}
        elif isinstance(obj, list):
            # clean all unused k/v pairs in api call
            for clean_obj in clean_up:
                if clean_obj in obj:
                    del obj[clean_obj]
            return [self.ref_clean_up(elem) for elem in obj]
        else:
            if isinstance(obj, str):
                return self.object_update(obj)
            else:
                return obj
    ...
    def long_substring(self, v_policy):
        """
    ...
    def policyset(self, policytype, objecttype):
        """
        ...
        if "rules" in policytype and policytype["rules"]:
            ...
            for k_policy, v_policy in rule_list.items():
                k_policy = self.long_substring(v_policy)
    ...
    def build_irule_config(self, config):
        """
        To create irule_config object for f5_conveter
        """
        custom_config_obj = {'irule_custom_config': config}
        return custom_config_obj


    def build_custom_config(self):
        """
        Loops through Keys and builds a list of policies and datascripts
        :return:
        """
        api = ApiSession.get_session(
            self.host, self.username, self.password, tenant=self.tenant,
        ...
        for k, v in api_results.items():
            if "HTTPPolicySet" == k:
                for policy_item in v:
                    for objecttype in policy_rule_types:
                        try:
                            results.extend(
                                self.policyset(policy_item[objecttype], objecttype))
                        except KeyError:
                            continue
            ...
            if "NetworkSecurityPolicy" == k:
                for policy_item in v:
                    results.extend(self.policyset(policy_item, "NetworkSecurityPolicy"))
        return self.ref_clean_up(results)
...
if __name__ == '__main__':
    ...
    custom_config_obj = CustomConfig(args.controller, args.username, args.password, 
