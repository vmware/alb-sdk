...
class F5Util(MigrationUtil):
    ...
    def add_complete_conv_status(
        ...
        for status in conv_const.STATUS_LIST:
            ...
            print(f"{status}: {len(status_list)}")
    ...
    def update_skip_duplicates(
        ...
        if dup_of:
            converted_objs.append({obj_type: f"Duplicate of {dup_of}"})
            LOG.info(
    ...
    def clone_pool(self, pool_name, clone_for,
        ...
        if new_pool:
            ...
            new_pool["name"] = f"{pool_name}-{used_pool[pool_name]}"
            if tenant:
    ...
    def update_pool_for_persist(
        ...
        if not pool_obj:
            LOG.error(
                "Pool %s not found to add profile %s",
    ...
    def clone_pool_group(self, pool_group_name, clone_for, avi_config,
        ...
        if new_pool_group:
            ...
            new_pool_group["name"] = f"{pool_group_name}-{used_pool_groups[pool_group_name]}"
    ...
    def write_status_report_and_pivot_table_in_xlsx(
        ...
        # xlsx workbook
        report_path = output_dir + os.path.sep + \
            f"{report_name}-ConversionStatus.xlsx"
    ...
    def clone_http_policy_set(self, policy, vs_name,
        ...
        clone_policy["name"] += f"-{vs_name}-clone"
        return clone_policy
        ...
        for index, rule in enumerate(vs_ds_rules):
            ...
            if rule_mapping and rule_mapping["type"] == "VSDataScriptSet":
                if "avi_config" in rule_mapping:
                    ...
                    ds_config["name"] = f"{rule}-{conv_const.PLACE_HOLDER_STR}"

                ds_config["tenant_ref"] = self.get_object_ref(tenant, "tenant")
                if prefix:
                    ds_config["name"] = f"{prefix}-{ds_config['name']}"
                existing_ds = [
                ...
                if "avi_config" in rule_mapping:
                    ...
                    if prefix:
                        policy_name = f"{prefix}-{rule}"
                    else:
                        policy_name = f"{rule}"
                    if not reuse_http_policy:
                        policy_name = f"{policy_name}-{vs_name}"
                ...
                if prefix:
                    policy["name"] = f"{prefix}-{policy['name']}"
    ...
    def remove_vs_names_when_vs_filter_is_provided(
        ...
        excel_path = output_dir + os.path.sep + f"{report_name}-ConversionStatus.xlsx"
        virtual_services = []
