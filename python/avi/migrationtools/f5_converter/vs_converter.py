...
class VSConfigConv(object):
    ...
    def convert_vs(self, vs_name, f5_vs, vs_state, avi_config, snat_config,
        ...
        if not app_prof:
            msg = f"Profile type not supported by Avi Skipping VS : {vs_name}"
            LOG.warning(msg)
            conv_utils.add_status_row(
                "virtual", None, vs_name, final.STATUS_SKIPPED, msg)
            return None
        ...
        if app_prof_obj and not (cme and oc_prof):
            ...
            app_prof_cmd = [
                obj for obj in (
                    sys_dict["ApplicationProfile"] +
                    avi_config["ApplicationProfile"]) if obj["name"] == f"{app_name}-cmd"]
            if app_prof_cmd:
                ...
                app_prof_cmd['name'] = f'{app_prof_cmd["name"]}-cmd'
        ...
        if vsvip_ref == "":
            msg = f"Skipped: Virtualservice {vs_name} has repeated vip not in " "different vrf"
            LOG.debug(msg)
            conv_utils.add_status_row(
                "virtual", None, vs_name, final.STATUS_SKIPPED, msg)
            return
        ...
        if not services_obj and not ip_addr and not vsvip_ref:
            msg = f"Skipped Virtualservice : {vs_name}"
            LOG.debug(msg)
            conv_utils.add_status_row(
                "virtual", None, vs_name, final.STATUS_SKIPPED, msg)
            return
        ...
        if pool_ref:
            ...
            if persist_ref:
                ...
                if self.prefix:
                    persist_ref = f"{self.prefix}-{persist_ref}"
            ...
            if (
                ...
                msg = f"Skipped VS : '{vs_name}' Secure persistence is applicable only \
                if SSL is enabled for Virtual Service"
                LOG.warning(msg)
                conv_utils.add_status_row(
                    "virtual", None, vs_name, final.STATUS_SKIPPED, msg)
                return
            ...
            if persist_type == "PERSISTENCE_TYPE_TLS" and \
                    app_prof_type == "APPLICATION_PROFILE_TYPE_SSL":
                msg = f"Skipped VS: '{vs_name}' \
                Only client-ip persistence is applicable for SSL VS"
                LOG.warning(msg)
                conv_utils.add_status_row(
                    "virtual", None, vs_name, final.STATUS_SKIPPED, msg)
                return
        ...
        if "policies" in f5_vs:
            if isinstance(f5_vs["policies"], str):
                vs_policies.extend(
                    [
                        f"{self.prefix}-{conv_utils.get_tenant_ref(f5_vs['policies'])[1]}"
                        if self.prefix
                ...
                vs_policies.extend(
                    [
                        f"{self.prefix}-{conv_utils.get_tenant_ref(name)[1]}"
                        if self.prefix else conv_utils.get_tenant_ref(name)[1]
        ...
        if not s_parts[0] == "0.0.0.0":
            ...
            if len(parts) > 1:
                mask = parts[1]
            policy_name = f"vs-{vs_name}-ns"
            if self.prefix:
                policy_name = f"{self.prefix}-{policy_name}"
        ...
        if snat_pool:
            if self.con_snatpool:
                ...
                msg = f"Skipped: snat conversion as input flag is not set for vs : {vs_name}"
                LOG.debug(msg)
        ...
        if vs_obj["application_profile_ref"]:
            ...
            if application_profile_obj and application_profile_obj[
                ...
                if not vs_obj.get("pool_ref", vs_obj.get("pool_group_ref")):
                    msg = f"Failed to convert L4 VS dont have pool or pool group ref: {vs_name}"
                    LOG.debug(msg)
                    conv_utils.add_status_row(
                        "virtual", None, vs_name, final.STATUS_SKIPPED, msg)
                    return
        ...
        if vs_ds_rules:
            ...
            if skipped_rules:
                skipped.append(f"rules: {skipped_rules}")
    ...
    def create_rule_action_for_via_header(self, via_header, via_request):
        ...
        via_header_key = f"{via_header}-{via_request}"
    ...
    def add_via_header_rule_to_http_policy(
        ...
        if http_policy:
            ...
            policy_set = {
                "name": f"{vs_obj['name']}-HTTP-Policy-Set",
    ...
    def app_profile_with_via_host_name(
        ...
        if f5_prof_obj and f5_prof_obj[0].get("via-host-name"):
            ...
            if via_request in ["append", "remove"]:
                if via_header_rule_dict.get(
                        f"{via_host_name}-{via_request}"):
                ...
                via_header_request = f"{via_host_name}-{via_request}"
                policy_name = None
                if pol_name:
