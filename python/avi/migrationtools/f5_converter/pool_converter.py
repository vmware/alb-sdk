...
class PoolConfigConv(object):
    ...
    def convert(
        ...
        for pool_name in pool_config.keys():
            ...
            if not f5_pool:
                msg = f"Empty pool skipped for conversion :{pool_name}"
                LOG.debug(msg)
                conv_utils.add_status_row(
                    "pool", None, pool_name, conv_const.STATUS_SKIPPED, msg)
                continue
            if "gateway-failsafe-device" in f5_pool:
                msg = f"Not supported gateway-failsafe-device,\
                    pool skipped for conversion :{pool_name}"
                LOG.debug(msg)
                conv_utils.add_status_row(
                    "pool", None, pool_name, conv_const.STATUS_SKIPPED, msg)
                continue
    ...
    def convert_for_pg(self, pg_dict, pool_obj, name,
        ...
        for priority in pg_dict:
            ...
            priority_pool_ref = f"{name}-{priority}"
            # Added prefix for objects
            if self.prefix:
                priority_pool_ref = f"{self.prefix}-{priority_pool_ref}"
