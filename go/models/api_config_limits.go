...
type APIConfigLimits struct {
	...
	// Maximum total number of API endpoints allowed across the system. Each ApiPath can have up to 7 ApiEndpoints. Field introduced in 32.1.4. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	NumApis *int32 `json:"num_apis,omitempty"`

	// Maximum allowed nesting depth of JSON schema definitions within an API policy. Schema structures that exceed this depth will be rejected at config time. In the datapath, JSON payloads with greater nesting depth will not be parsed. Field introduced in 32.1.4. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
