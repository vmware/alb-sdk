...
// HTTPSecurityAction HTTP security action
...
type HTTPSecurityAction struct {
	...
	// File to be used for generating HTTP local response. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	File *HTTPLocalFile `json:"file,omitempty"`

	// Secure SSL/TLS port to redirect the HTTP request to. Allowed values are 1-65535. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	HTTPSPort *uint32 `json:"https_port,omitempty"`

	// Rate limiting configuration for this action. Field introduced in 18.2.9. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
