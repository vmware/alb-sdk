...
type OpenStackConfiguration struct {
	...
	// When set to True, OpenStack resources created by Avi are tagged with Avi Cloud UUID. Field introduced in 21.1.5. Allowed in Enterprise edition with any value, Enterprise with Cloud Services edition.
	...
	// When set to True, VIP ports are created in OpenStack tenant configured as admin_tenant in cloud. Otherwise, default behavior is to create VIP ports in user tenant. Field introduced in 21.1.5. Allowed in Enterprise edition with any value, Enterprise with Cloud Services edition.
