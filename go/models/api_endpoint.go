...
type APIEndpoint struct {
	...
	// Additional labels for this endpoint, applicable when the endpoint is an active API. By default, this is applied in conjunction with the global active_api_labels defined in ApiPolicy. Set override_global_active_api_labels to true to use only the endpoint-level labels. Field introduced in 32.2.1. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	ActiveAPILabels *APILabels `json:"active_api_labels,omitempty"`

	// Endpoint definition is deactivated. When true, this endpoint will not be validated, learned, or labeled. Field introduced in 32.2.1. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	...
	// When true, only the endpoint-level active_api_labels are applied to this endpoint, ignoring the global active_api_labels defined in ApiPolicy. When false (default), endpoint-level and global active_api_labels are applied in conjunction. Field introduced in 32.2.1. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	...
	// Expected response definitions for this endpoint, each associated with an HTTP status code or status code range. Field introduced in 32.2.1. Maximum of 32 items allowed. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
