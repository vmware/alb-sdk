...
type IPAMDNSInternalProfile struct {
	...
	// List of service domains. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	DNSServiceDomain []*DNSServiceDomain `json:"dns_service_domain,omitempty"`

	// Avi VirtualService to be used for serving DNS records. It is a reference to an object of type VirtualService. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	DNSVirtualserviceRef *string `json:"dns_virtualservice_ref,omitempty"`

	// Default TTL for all records, overridden by TTL value for each service domain configured in DnsServiceDomain. Allowed values are 1-604800. Unit is SEC. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
