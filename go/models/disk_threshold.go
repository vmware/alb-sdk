...
type DiskThreshold struct {
	...
	// Trigger the action when disk usage percent exceeds on the specified path. Allowed values are 1-25. Field introduced in 31.1.1. Unit is PERCENT. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	MaxDiskPercent *uint64 `json:"max_disk_percent,omitempty"`

	// Trigger the action when total available diskspace falls below this level. Allowed values are 5-50. Field introduced in 31.1.1. Unit is PERCENT. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
