...
type LockoutConstraint struct {
	...
	// Time window for evaluating failed attempts in seconds. Defaults to 900 seconds. Allowed values are 300-1800. Special values are 0 - Do not reset login failure counts on the basis of any evaluation window.. Field introduced in 31.3.1. Unit is SEC. Allowed with any value in Enterprise, Enterprise with Cloud Services edition.
	LockoutEvaluationPeriod *uint32 `json:"lockout_evaluation_period,omitempty"`

	// Number of failed attempts before account lockout. Defaults to 3. Setting it to 0 allows unlimited login failure attempts without any lockout. Allowed values are 0-5. Field introduced in 31.3.1. Allowed with any value in Enterprise, Enterprise with Cloud Services edition.
