...
type SecurityMgrDebugFilter struct {
	...
	// Cooldown period between adaptive sampling configuration sends to prevent excessive updates. Allowed values are 1-600. Field introduced in 32.1.4. Unit is SEC. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	AdaptiveSamplerConfigCooldown *uint32 `json:"adaptive_sampler_config_cooldown,omitempty"`

	// Adaptive sampler tick interval for periodic sampling adjustments. Allowed values are 1-3600. Field introduced in 32.1.4. Unit is SEC. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	AdaptiveSamplerTickInterval *uint32 `json:"adaptive_sampler_tick_interval,omitempty"`

	// Custom API path markers for endpoint classification (e.g., /api/, /v1/, /graphql). If not configured, uses default markers. Field introduced in 32.1.4. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	...
	// [Internal] Periodicity at which Orphan/Zombie/Active API determination routine runs. Allowed values are 1-10080. Field introduced in 32.1.4. Unit is MIN. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	SecmgrAPIClassificationTaskPeriodicity *uint32 `json:"secmgr_api_classification_task_periodicity,omitempty"`

	// [Internal] Periodicity at which Orphan/Zombie/Active API hits population routine runs. Allowed values are 5-1440. Field introduced in 32.1.4. Unit is MIN. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	SecmgrAPIHitsPopulationInterval *uint32 `json:"secmgr_api_hits_population_interval,omitempty"`

	// Trigger full sync of API specification changes to learning database for all eligible VSes. Field introduced in 32.1.4. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	SecmgrWaapFullSync *bool `json:"secmgr_waap_full_sync,omitempty"`

	// Trigger full sync for a specific VS UUID. If set, only this VS will be processed. Field introduced in 32.1.4. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	SecmgrWaapFullSyncVsUUID *string `json:"secmgr_waap_full_sync_vs_uuid,omitempty"`

	// File extensions considered as static non-API content (e.g., .html, .css, .js, .png). If not configured, uses default extensions. Field introduced in 32.1.4. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
