...
type WaapPathTreeLabel struct {
	...
	// True if this label is attached directly on this endpoint (override_global_labels), overriding the ApiPolicy/label-profile-wide label set. False if inherited from that global set. Field introduced in 32.1.4. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	LocalOverride *bool `json:"local_override,omitempty"`

	// Label name. Field introduced in 32.1.4. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
